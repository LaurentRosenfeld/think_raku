...
about the Raku language in particular.
...
Another possibility might be to use a feature of Raku which 
...
languages, including Perl~5, from which Raku is derived 
in part). In Raku, the operators implementing this 
...
two integer numbers is sufficient for the Raku compiler 
...
of Raku returns a reversed version of a string and would 
...
This might be implemented in Raku with the following recursive 
subroutine:
...
use of the fact that subroutines have lexical scope in Raku 
...
As is often the case in Raku, and even more so with 
...
\begin{verbatim}
my $string = "Four score and seven years ago our fathers ...";
...
We can code a ROT13 in a simple Raku one-liner (see Section~\ref{one-liner mode}):
\index{one-liner mode}
...
Raku that the \verb'@queue' parameter is an array of arrays. 
...
Raku is that it gives you a modern and powerful tool to use 
...
of Raku, which returns a sequence of unique values 
...
Note that Raku also has a {\tt repeated} built-in 
...
Note that Raku has a {\tt roll} built-in that returns 
...
inserting items at the start of the array, Raku has 
...
fact that, in Raku, subroutines are \emph{closures}, 
...
a Raku module, i.e., in a separate file that will be 
...
Now a Raku program will be able to load this module 
...
Raku has a list of places to look for modules, 
which may vary from one Raku installation to another. 
The first line {\tt use lib ".";} tells Raku to also 
...
second line {\tt use BisectSearch;} tells Raku to 
...
\url{https://docs.raku.org/routine/trans}) might speed up 
...
\subsection{Exercise in Section~\ref{given_when}: the {\tt given ... when} Statement (p.~\pageref{proceed_ex})}
...
Raku compiler to distinguish between the two operators.
...
\url{https://doc.raku.org/language/testing}.
...
classical problem, but Raku actually provides a method for that 
