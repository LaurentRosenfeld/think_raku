...
\end{verbatim}
%
The header for this definition indicates that this type is a new class is which is called {\tt Point2D}.
The body for this definition has two defining attributes, i.e., named properties 
associated with the class, here named the abscissa and ordinate 
...
Much like a manufacturing factory can create physical objects, 
This software type object is like a factory for creating software objects.  To create 
a point, you call the {\tt new} method on the {\tt Point2D} class using the dot notation:
...
\index{isa method}
\index{WHAT}
Of course, you can create as many points as you wish.

\index{constructor}
\index{new, object constructor}
\index{dot notation}
\index{object!constructor}
The {\tt new} method is called a {\bf constructor} because it is a way
to construct or create a new object of this type. In this case, it has not 
been defined in this example; this is not necessary because 
Raku supplies a default {\tt new} constructor method for 
every class (we'll see later how). Using the dot notation as the method invocation 
syntax is the same notation we have 
used throughout this book to invoke built-in methods. Even when Raku creates a 
default {\\ new }} method, you are not forced to use this constructor; you can also create 
your own (and you may name it {\tt new} or some other name you prefer), 
but we will stay with the using the built-in {\tt new} method provided by Raku for the 
time being.
\index{invocation!method}
\index{method invocation}

Creating a new object with a class is called {\bf instantiation}, 
and the object which is created is an {\bf instance} of the class.
\index{instance}
\index{object!instance}
\index{instantiation}

Every object must be an instance of some class, so the terms 
...
Each attribute of the {\tt Point2D} class should be limited to refer to a 
