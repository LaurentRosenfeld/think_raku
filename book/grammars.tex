...
Raku offers ways to group and name regex patterns so that 
...
JSON or YAML documents, or even computer programs: Raku 
programs are actually parsed using a Raku grammar written 
in Raku.
...
efficient. Raku offers all the tools to do that 
very easily.
...
With a grammar and Raku's ability to create its own operators, 
...
if "abc\ndef\n" ~~ / <first=line> <second=line> / {
    ...
    print $_.chomp for $<line>.list;      # -> abcdef
}
\end{verbatim}
...
For example, any Raku program is parsed and executed 
using a Raku grammar written in Raku, and you could write a grammar 
...
scare you off grammars. Raku grammars are nothing complicated; 
...
The {\tt is Message} trait in the header tells Raku that 
...
the {\tt ArithmActions} class, which tells Raku which 
...
(basically corresponding to hashes in Raku);
...
Take a look at the grammar of the Raku {\tt JSON::Tiny} 
...
extending the Raku language itself. It is possible, for 
example in the context of a module or a framework, to ``subclass'' 
the standard Raku grammar, i.e., to write a new child grammar that 
inherits from the standard Raku grammar, but adds a new 
feature, overloads an operator, or modifies some other syntax 
element, and to run this program with the same Raku compiler,
...
Remember also that the Raku debugger (see 
Section~\ref{raku-debugger}) can be very helpful. We 
...
grammatical analysis of structured text. In Raku, 
...
\index{match object}
\item[Match object] in Raku, an object (of type {\tt Match}), 
...
by Raku, whereas the AST contains information deemed useful and 
